...
switch($position){
case 'Admin':
$result=$con->query("SELECT admin_id, username FROM admin WHERE username='$username' AND password='$password'");
...
break;
case 'Pharmacist':
$result=$con->query("SELECT pharmacist_id, first_name,last_name,staff_id,username FROM pharmacist WHERE username='$username' AND password='$password'");
while ($row=$result->fetch()) {
if($row>0){
session_start();
...
break;
case 'Cashier':
$result=$con->query("SELECT cashier_id, first_name,last_name,staff_id,username FROM cashier WHERE username='$username' AND password='$password'");
while ($row=$result->fetch()) {
if($row>0){
session_start();
...
break;
case 'Manager':
$result=$con->query("SELECT manager_id, first_name,last_name,staff_id,username FROM manager WHERE username='$username' AND password='$password'");
while ($row=$result->fetch()) {
if($row>0){
session_start();
