...
if(isset($_POST['ok']))
{
	...
	
	{
		...
		{		
			...
			{
				if ($form_action==="entraineur") {
					$req='INSERT INTO entraineur(pseudo,nom,prenom,ddn,mdp) VALUES(?,?,?,?,?)';
				}
				if ($form_action==="journaliste") {
					$req='INSERT INTO journaliste(pseudo,nom,prenom,ddn,mdp) VALUES(?,?,?,?,?)';
				}
				if ($form_action==="admin") {
					$req='INSERT INTO admin(pseudo,nom,prenom,ddn,mdp) VALUES(?,?,?,?,?)';
				}
						...
						<?php
	   			 		echo '<meta http-equiv="refresh" content="0; url=../news/newnews.php">';
						exit ();
        							}
